...
- Expert triage rule (mandatory): first compile up to 50 candidate stories from the last 24-48 hours, then deduplicate/merge same-topic items and rank by (1) cross-source citation frequency, (2) source authority, (3) implementation value.
- ByteByteGo priority rule: if ByteByteGo has a new article and it ranks inside the top-50 triage pool, prefer it as the primary topic.
