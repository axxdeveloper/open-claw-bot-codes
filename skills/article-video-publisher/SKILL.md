...
description: Turn high-signal AI, backend, and technical topics into Traditional Chinese narrated explainer videos and publish them to YouTube. Use for workflows that need Codex 5.5 xhigh source triage, depth planning, comprehension review, source-attributed slides, Taiwan Mandarin narration, video validation, Shorts fallback, and public YouTube upload.
---

# Article Video Publisher

## Goal
Produce one public YouTube explainer from exactly one high-signal original technical, AI, backend/software, engineering-operator source article, video, report, release, repo, paper, or technical data source:
...
Do not turn a source-introduction video into a meta lesson about how to read sources. The video itself is already doing the reading work for the viewer. Avoid slide titles, narration, or summaries framed as `怎麼讀這篇文章`, `怎麼看來源`, `怎麼讀 benchmark`, `如何判讀來源`, `讀文章時要看...`, `看這篇時...`, `先看來源：這篇在回答什麼`, `讀這篇時要保留...`, `看完這篇...`, `來源能說的/不能說的`, or similar generic reading-method/source-reading advice unless the original source is explicitly about reading methodology, evaluation literacy, or benchmark interpretation as its main subject. When a source needs context, benchmark, or evidence caveats, state the source-native facts directly in the title/body: what the source says, what changed or was measured, what conditions apply, what conclusion is supported, and what cannot be inferred. For example, prefer `這個 benchmark 只比較 DocKQ，不等於所有蛋白任務` over `怎麼讀來源的 benchmark 敘述才不會過度解讀`; prefer `Biohub 同時發布 ESMC、ESMFold2 與 Atlas` over `先看來源：這篇在回答什麼`.
...
- `observed_problem`: the TodayShip problem this addresses, such as a cover that looks like a body slide, source URL truncation, abrupt audio starts, cluttered slides, unclear first 30 seconds, or weak source-understanding value.
...
- Claude Code slide workflow adaptation from 2026-05-31: treat the strongest Claude Code slide examples as a process lesson, not a model switch. Good agent-made decks are usually programmatic, previewed, screenshot-reviewed, and governed by a reusable design system. For TodayShip, apply this as follows:
  - For non-trivial technical videos, create a structured visual brief before full slide generation: `main_object`, `source_specific_anchor`, `layout_family`, `visual_role`, `diagram_spec_or_source`, `mobile_readability_risk`, and `expected_screenshot_check`.
  - For architecture/dataflow/state/timeline/failure-path slides, use renderable structure when feasible: `diagram_spec`, `diagram.nodes`/`diagram.edges`, `mermaid_code`, `graphviz_code`, or `svg_source`. A prose-only `visual` field is acceptable for simple editorial images, but repeated structural diagrams without specs should trigger a fix or a recorded pilot exception.
  - Body explanation slides default to dark mode: deep neutral background, high-contrast Traditional Chinese text, restrained accent colors, and clear source footer contrast. If a source screenshot, chart, or code block needs a light surface, place it inside a framed light panel on the dark slide instead of switching the whole deck back to a white presentation style.
  - For hard topics, generate three source-specific visual direction previews before the full deck: one safe editorial direction, one bolder source-native direction, and one wildcard. Each preview should include a cover treatment plus one representative body/mechanism slide. Pick the direction by source specificity, readability, and comprehension value; do not ask Isaac unless the workflow is manual or the choice affects public brand.
  - Do not let one polished overview image carry an entire teaching segment. If a slide would need more than about 35 seconds of narration, prefer a progressive reveal or multiple slides. A static slide over 45 seconds is a blocker before upload unless the artifact is explicitly approved for a manual exception.
  - Render a contact sheet or screenshot set before upload/release and inspect it like a UI bug hunt: overlaps, tiny text, low contrast, source-footer collisions, repeated layouts, missing main object, placeholder labels, and weak diagram geometry. If issues are found, fix and rerender the affected slides; if no issues are found, record what was checked.
  - This is an upload gate, not only an advisory note. New article-video artifacts must write `review/slide-visual-workflow.md`, create `review/slide-contact-sheet.png`, and pass `scripts/check_slide_visual_workflow.py` before upload. Hard topics need three preview artifacts under `review/visual-direction-previews/` or an equivalent recorded preview set before full-deck rendering.
  - Do not replace the whole render path with HTML/CSS yet. Pilot HTML/CSS or Reveal-style rendering on the first 4 slides of one future artifact only, and promote it only if screenshot QA shows clearer diagrams without hurting mobile readability or production reliability.
...
## Routine Scope Boundaries
小舟 article-video workflows should stay in the technical/AI/backend/software/operator lane. Scheduled runs, source-watch selection, HN-discovered topics, fallback/補位 workflows, and generic "find a good article" workflows should not use unrelated topics as filler.

Do not create separate 小舟 article-video workflows for unrelated topic lanes.
...
- If YouTube Studio's content list still shows stale upload rows for that already-created artifact but local process/session checks show no active upload owner and oEmbed/metadata identify the existing video, the row alone is not a global blocker. Do not click `取消上傳` or delete without Isaac's approval; isolate the stale row in same-video bookkeeping and allow a separate reviewed source to upload after fresh duplicate, quota, profile/account, browser cleanup, active-task, and public-result checks.
...
Before writing slides or narration, assume viewers are hearing the topic for the first time. Do not assume they know the source, acronym, product, vulnerability class, benchmark, research method, platform feature, or technical term.
...
5. `concrete_scenario`: one workflow, incident shape, query, agent trace, or other concrete case.
...
- `example_or_scenario`: one concrete example, incident shape, workflow, or operational scenario.
...
- `source.json.topic_domain` must be exactly `technical`, never null/empty. Put finer labels such as `backend_database`, `technical_ai_backend`, `infra_sre`, or `operator_interview` in `source.json.topic_subdomain`.
...
- Record the de-noise pilot with canonical keys in `review/improvement-notes.md`: `opening_new_labels_before_30s: <int>`, `opening_slide2_points_count: <int>`, `opening_failure_signal_before_seconds: <float>`, and `opening_source_identity_before_seconds: <float>` (targets: labels/points `<=2`, first failure signal `<=12s`, source identity `<=12s`).
...
- Failure-signal-first pilot rule: if a fresh 5-sample audit already has `opening_new_labels_over_1_count<=1` but still has `opening_failure_signal_after_8s_count>=2`, treat late failure-signal timing as the primary bottleneck (not jargon density). Run one next-video pilot with `opening_new_labels_before_30s<=1` plus `opening_failure_signal_before_seconds<=8`, keep packaging variables frozen for that pilot, and rerun pre/post audit before changing packaging rules.
...
- Domain-alignment rule: because `topic_domain` is `technical`, the opening must name the source object, technical context, and core term/problem. Do not use the opening to list roles or promise a workflow/risk-check unless the source itself is about that workflow.
...
Reject or reshape the topic if the value proposition is vague, only says "了解新聞/數字", or cannot name what the viewer will understand better about the original source. It is acceptable for the value to be simply "this difficult source becomes easier to read or hear."
...
At the start of automatic scheduled topic selection, choose the strongest source-backed candidate inside 小舟's routine scope: technical, AI, backend/software, or engineering-operator. Use source quality and source diversity together so the channel does not over-rely on one domain or source family.
...
Also read already-available technical community signals when selecting technical AI/backend topics, but only as discovery context for what technical communities are noticing. Do not create a proactive social follow, polling, or engagement workflow for topic radar. Public posts from technology workers, KOLs, maintainers, researchers, and operators can suggest attention around AI engineering, backend/software, databases, Kafka/Cassandra, streaming/messaging, distributed systems, infra/SRE, security, devtools, coding agents, and runtimes. A social post is not the public source; if selected, trace the signal to the original article, official release/blog, repo, paper, benchmark, talk, or docs before making a video.
...
- Important AI interview topics include frontier lab leaders, researchers, model/inference/tooling builders, safety/eval voices, and AI infrastructure operators. Do not turn broad celebrity, biography, or business-positioning interviews into videos unless they explain a concrete engineering mechanism or tradeoff.
...
- Treat AI product/operator interviews as a separate high-signal lane when a credible builder explains how AI changes product design, user experience, team structure, management layers, hiring, consumer AI, or company operating models. Select them only when the episode has a concrete mechanism or source-backed experience that can be explained, not just founder biography, motivation, or company promotion.
...
### Companion Shorts from published long-form
- Do not let Shorts stay only as a review/bookkeeping note. Every new published long-form artifact must run a companion Shorts fit check after public verification. When it has a concrete `shorts_single_learning_point_note`, it is eligible for one local companion Shorts pilot; when it does not, record the plain reason (`no_viable_single_point_short`, `too_context_heavy`, `source_risk`, `vertical_text_cram`, etc.) in `review/shorts-vs-longform-fit.md` or the pilot/backlog notes.
- Start with local-only pilots unless Isaac has explicitly authorized automatic public Shorts publishing. A pilot should create a run directory under `reports/youtube-shorts-pilot/YYYY-MM-DD/<slug>/`, write `source.json`, `script.md`, `youtube-metadata.json`, review notes, and render a local vertical candidate without opening YouTube Studio.
- Companion Shorts pilot bookkeeping must be explicit: every local pilot directory also writes `status.md` and `final-report.md` with source URL, parent long-form URL, single learning point, rendered candidate path, duration, loudness result, and `public_upload` state. Do not leave pilot outputs as media files only.
- A companion Short must keep exactly one learning point from the parent video, name the same original source, and add the parent long-form URL in metadata. It should not compress the whole long-form into a dense teaser.
- Keep the narration tighter than long-form but still comfortable and understandable. Do not solve Shorts by globally speeding up speech or cramming a full long-form summary into a fast teaser; cut scope instead.
- Render companion Shorts as 1080x1920 vertical, target roughly 45-90 seconds when possible, keep under 180 seconds, add a short visual lead-in, use OpenAI TTS, and run at least ffprobe plus loudness checks before reporting a candidate.
- Public upload remains a separate step: rerun account/profile, quota, duplicate, Content ID risk, source traceability, and browser cleanup guards before opening Studio. Do not upload the first pilot publicly without explicit Isaac approval for the new Shorts public lane.
...
reports/article-video-publisher/YYYY-MM-DD/<slug>/
  ...
  review/slide-visual-workflow.md
  review/slide-visual-workflow-check.json
  review/slide-contact-sheet.png
  review/slide-contact-sheet.json
...
- `article_date`
- `topic_domain` exactly `technical`
- `topic_subdomain` for finer source classification, for example `backend_database`, `ai_agents`, `infra_sre`, or `operator_interview`
- `accessed_at`
- `topic_reason`
- `reader_value`
- `candidate_score`
...
- Explanation/body slides should default to dark mode. Use a deep neutral background, strong text contrast, and a small consistent accent palette; source footers must remain readable. For source screenshots, charts, tables, or code that need light backgrounds, use a light inset panel within the dark composition rather than switching the whole slide to white.
- Do not let body slides collapse into the same rounded-box/card + bullet pattern. Each explanation unit should choose a source-specific visual treatment: annotated source excerpt, architecture cutaway, state transition, before/after contrast, metric chart, timeline, topology map, failure path, concrete example scene, or evidence-boundary matrix. If card/board/checklist/question/summary-style layouts take over about one third of content slides, rewrite the plan before rendering.
- Add a diagram-first planning layer for technical mechanism slides. Before rendering slides for architecture, dataflow, state transitions, timelines, topology, sequence flows, or failure paths, write a source-auditable diagram spec instead of only a prose visual idea. Prefer code-drawn diagrams when the concept is structural: Mermaid for simple flowcharts/sequences/state diagrams, Graphviz for dependency/topology graphs, or small custom SVG/HTML/CSS/canvas when exact layout, icons, callouts, or Taiwan Traditional Chinese typography matter. Keep labels source-backed and precise; do not invent components just to make a diagram look complete.
- Treat Mermaid/Graphviz as a diagram source, not as a visual style by itself. Use it when it clarifies node/edge relationships, sequence, state, or failure path. Do not use it for slide 1 covers, concrete object comparisons, UI screenshots, metric charts, or cases where a custom annotated illustration/source excerpt would teach better.
- In `plan_payload.json`, structured diagram slides should include enough machine-readable source to audit and rerender the diagram, for example `diagram_spec`, `diagram.nodes`/`diagram.edges`, `mermaid_code`, `graphviz_code`, or `svg_source`, plus a short note mapping the diagram to source claims. Rendered PNG/SVG assets should be saved under the artifact's `slides/` or `assets/` directory with names that match slide numbers.
- In `plan_payload.json`, slides 1-4 and every architecture/dataflow/state/timeline/topology/sequence/failure-path slide must include a `visual_brief` object with these string fields: `main_object`, `source_specific_anchor`, `layout_family`, `visual_role`, `diagram_spec_or_source`, `mobile_readability_risk`, and `expected_screenshot_check`. This is the Claude-Code-style planning layer that keeps the picture tied to the source before rendering.
...
- Treat single-slide narration length as a design signal. If one static slide is expected to carry more than about 35 seconds, split it into a progressive reveal or multiple visual beats. After rendering, any `video/build/segments/*.mp4` segment over 45 seconds must be fixed before upload.
...
- Cue-pilot readiness gate: before testing intro/outro cues on a new technical long-form artifact, require all five on that artifact: `review/narration-speed-check.json` PASS with no warnings, `review/preanalytics-opening-check.json` has `opening_failure_signal_before_seconds<=8`, `review/improvement-notes.md` records `opening_new_labels_before_30s<=1`, `review/audio-check-gate.json` PASS with no onset-edge warnings, and `first_spoken_onset_seconds` in the comfort target (`1.35-1.80s`). If any item fails/is missing/warns, keep `decision=no_background_music` and prioritize pacing/opening clarity fixes first.
- Batch readiness rule for quality-learning: if a fresh 4-5 sample audio batch shows `no_background_music_count == sample_size` and (`cue_trial_rejected_count + cue_trial_not_used_or_deferred_count >= sample_size - 1`), classify as `cue_pilot_readiness_gap`. The next sprint should produce a concrete low-risk cue pilot plan (track shortlist, mix range, intro/outro timing) but must not force production cue usage until the readiness gate passes.
...
- if the source is an interview/podcast/video episode, the script explains a concrete AI mechanism, tradeoff, safety/eval issue, or engineering lesson rather than becoming a personality profile
...
- the script does not use source-specific acronyms, product names, benchmark names, exploit classes, model/eval terms, or technical terms as if the viewer already knows them
...
- `source.json.topic_domain` is exactly `technical`, with any finer source category recorded in `source.json.topic_subdomain`, and matches the source object being introduced
...
- body slides must not repeat one template across the deck, especially the left-diagram/right-bullets or three-card pattern. Treat repeated card/checklist/summary/question layouts as a comprehension failure, because the image stops doing explanatory work.
- architecture/dataflow/state/timeline/failure-path slides include a structured diagram source when feasible (`diagram_spec`, `diagram.nodes`/`diagram.edges`, `mermaid_code`, `graphviz_code`, or `svg_source`) so the visual can be audited and rerendered; a prose-only `visual` field is a warning for structural diagrams
...
- This final gate also blocks upload if the artifact already has `youtube/upload-result.json`, or if any unresolved `youtube/duplicate-public-upload-blocker.json` exists under `reports/article-video-publisher/**`. Existing uploads must use same-video verification/bookkeeping recovery only; unresolved public duplicates must be hidden/unlisted/private/deleted with Isaac approval or explicitly marked resolved before any new Studio upload.
- Before clicking Publish in YouTube Studio, verify upload candidate identity against loudness evidence. The basename selected/uploaded in Studio must match a `PASS` entry in `review/loudness-check.json` (or the designated replacement loudnorm candidate). If both `video/<slug>.mp4` and `video/<slug>-loudnorm.mp4` exist and Studio points to the non-PASS variant, stop and correct the upload target before any Public action.
...
- For quality-learning audio/sound-design trend audits, if a fresh 5-sample batch shows `pre_normalization_loudness_fail_count>=3` or `uploaded_file_audio_mismatch_incident_count>=1`, run one next-video audio hardening pilot before promoting other sound-design changes: (a) run `check_video_loudness.py` on the raw render and again on the upload candidate, (b) ensure `youtube/metadata.json` `video_path` and the Studio-selected upload basename match a loudness-PASS file, and (c) record `upload_target_identity_check: PASS|FAIL` in `review/improvement-notes.md` plus the quality-learning audit JSON.
...
- For final-review quality-learning audits on already-published samples, if a fresh 5-sample batch is `thumbnail-followup PASS=5/5` and `pre-analytics Mobile readability PASS=5/5` but still has `cover_text_density_risk >=4/5`, classify it as `frontdoor_density_scorecard_misalignment` (PASS drift, not closure). Keep front-door packaging as an active bottleneck, require quantified `cover_alias_next_fix` (`title+subtitle<=54`, `subtitle<=1 line`) on the next production candidate, rerun `check_thumbnail_followup_note.py` before closeout, and do not report finished-review all-clear until that warning count drops below `3/5`.
- For final-review quality-learning audits on already-published samples, if a fresh 5-sample batch is `cover_text_density_risk=5/5` and `cover_text_density_unquantified_fix>=2/5`, classify it as `frontdoor_density_quantified_fix_gap`. Before calling trend improvement, backfill measurable `cover_alias_next_fix` targets on every warned sampled artifact (`title+subtitle<=54`, `subtitle<=1 line`, move detail to slide 2+) and rerun `check_thumbnail_followup_note.py` on those artifacts so the next trend audit is based on quantified fixes, not free-text suggestions.
- For final-review quality-learning audits on already-published samples, if `check_post_publish_review_card.py` shows any missing card file in `published_public_verified` samples, classify it as `post_publish_card_coverage_gap` and repair missing cards before using that sampled batch for trend-closure decisions.
...
- Before upload, run `/Users/openclaw-user/.openclaw/workspace/scripts/check_slide_visual_specificity.py <artifact_dir> --json-out <artifact_dir>/review/slide-visual-specificity-check.json`. Treat `FAIL` as a blocker (missing/invalid early-slide `visual` fields in `plan_payload.json`, or too many repeated card/bullet/box layouts across body slides).
- Before upload, create a slide contact sheet and run the Claude-Code-style visual workflow gate:

```bash
python3 /Users/openclaw-user/.openclaw/workspace/scripts/make_slide_contact_sheet.py <artifact_dir> \
  --output <artifact_dir>/review/slide-contact-sheet.png \
  --json-out <artifact_dir>/review/slide-contact-sheet.json
python3 /Users/openclaw-user/.openclaw/workspace/scripts/check_slide_visual_workflow.py <artifact_dir> \
  --json-out <artifact_dir>/review/slide-visual-workflow-check.json
```

Treat `slide-visual-workflow-check.json` `FAIL` as a blocker. The note `review/slide-visual-workflow.md` must record `visual_brief_created: PASS`, `visual_brief_fields_complete: PASS`, `structured_diagram_specs_checked: PASS`, `progressive_split_reviewed: PASS`, `slide_duration_reviewed: PASS`, `dark_mode_body_slides_reviewed: PASS`, `direction_previews_considered: PASS` for hard topics, `direction_preview_count: 3`, `contact_sheet_created: PASS`, `contact_sheet_path: review/slide-contact-sheet.png`, `contact_sheet_reviewed: PASS`, `screenshot_review_findings: ...`, `fix_cycle_completed: PASS|NO_ISSUES_FOUND`, `mobile_readability_reviewed: PASS`, and `template_repetition_reviewed: PASS`. If the render path does not preserve `video/build/segments/*.mp4`, also record `max_static_slide_seconds: <number>` and `longest_static_slide: <slide number/title>`.
...
- If `review/slide-visual-specificity-check.json` warns `visual_field_missing_or_generic` or `low_visual_layout_diversity`, upload remains eligible but rewrite the warned slide `visual` text to source-specific scene details before workflow closeout when feasible. If it fails with `repeated_card_bullet_layout_pattern`, do not upload; replace repeated cards/bullets with visual families that explain source mechanisms, evidence, examples, limits, or failure paths.
...
- For quality-learning audits on already-published samples, if a fresh 5-sample batch shows `repeated_card_bullet_layout_pattern FAIL >=3/5` while `thumbnail-visual-direction PASS=5/5` and `first60-density PASS=5/5`, classify it as `source_visual_mechanism_debt` (not an opening or cover bottleneck). In that case, the next production candidate must run a pre-render visual rewrite pass: replace at least 3 card/bullet-like body slides with source-specific visual families (state transition, before/after, metric chart, failure path, topology, timeline) and rerun `check_slide_visual_specificity.py` before render and before upload.
...
- If viewers reply to a YouTube video, YouTube Community post, Blogger post, or any other active public surface
