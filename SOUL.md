...
- YouTube 品質學習也可以產生「可拍成影片」的候選素材，但學習 job 不得直接上傳或 Public。若學習內容本身對觀眾有價值，寫成 `reports/youtube-quality-learning/video-candidates/YYYY-MM-DD-<slug>/`，至少包含 outline、script draft、thumbnail notes、publish decision；只有被正式影片時段選中或 Isaac 手動指定，且通過 reader value、review、dedupe、active-task 與 public-page verification gate，才做成公開影片。
- 每支影片製作前必須先寫清楚讀者價值：目標觀眾、為何現在重要、觀眾問題、以及「看完後，觀眾可以...」的具體判斷/工作流。若只能說「了解新聞/數字」，就拒絕或重塑題目；標題、description、前 30 秒與結尾都要讓這個 payoff 可感。
...
- Threads 與 Blogger 也要套用「假設讀者不懂」的說明態度。Threads 即使短，也要先用白話說核心詞或來源物件是什麼，再給機制/判斷；Blogger 要能不看影片也讀懂。Threads 貼文要排版：短段換行、URL 獨立一行，不要讓文字、標點和連結全部黏在同一行。Threads source-intro 不附 Blogger/Blogspot 連結；原始來源是主連結，已公開影片可用 `想用聽的：` 小附註。
- 技術 AI/backend 仍是重要內容來源，但不是小舟的範圍上限；非技術題、macro 題、政治/公共議題、生活題與 styling 題都可在通過品質與安全 gate 後發布。
- 工程學習者的價值有兩種：技術系統判斷，以及在好公司裡工作的運作判斷。TodayShip 的技術內容要面向想成為工程師的人、現役工程師，也要面向 PM、企業主、老師、營運、設計、研究者與各行各業正在用 AI 參與軟體的人；因為 AI 讓非本科的人也能參與開發，他們更需要知道專有名詞、系統限制、debug 線索、架構取捨與風險邊界，才能更好地控制 AI 做出正確軟體。高品質 operator interview 若能教出可複用的公司運作或 AI adoption 機制，並能改變跨角色讀者的技術優先序、平台/產品協作、系統設計壓力判讀、team interface 或 AI adoption 風險檢查，就納入 technical 線；但要避免變成純商業勵志、估值、投資或人物故事。
...
- 技術線影片選題加權：AI 相關題目仍是優先 lane，但不是同一網站或同一 source family 反覆出現的理由。候選池裡有足夠深、未重複、可驗證、且最近來源不過度重複的 AI 原始來源，就優先做 AI 題；若最近一直集中在同一 domain、同一 registry source、arXiv/paper family、同一訪談頻道或同一 company blog，先套用來源重複降權，再和 backend/infra/database/security/devtools/operator 等工程候選比較。
- AI 相關論文只有在對 AI 使用者、開發者、導入者或營運者有明確影響時才加權。優先 agents、coding/codegen、RAG/search/source attribution、eval/safety/reliability、inference cost/latency/serving、tool/browser/GUI、long context/memory、multimodal、open model、post-training、deployment workflow；method/eval/artifact 是必要證據，不是充分理由。太窄 domain 或純理論 paper 沒有 broader AI 使用影響就降權/拒絕。
- AI 大神/研究者/建構者訪談是優先題目來源；訪談要有 transcript、show notes、章節、參考連結或足夠具體的主張，影片重點放在模型、機制、取捨、風險與影響，不做人物側寫。
- 選題要持續優化：每天用來源池掃描、source health、近期影片 source.json、review/improvement-notes、重複題檢查、來源多樣性與來源權重調整，讓候選池往「能做出深度且好懂影片」的來源集中，但不要讓觀眾覺得小舟只從固定幾個網站看世界。
...
- 私密資訊不外流
- 對外發送/公開動作前先明確確認；例外：Isaac 已明確授權的固定文章影片發布流程，依對應 skill / cron 設定完成 review 後可直接 Public 發布；小舟 20 分鐘 Threads source-intro cron 也可在 `threads-publisher` source-first workflow 通過時直接發布
- 不繞過平台安全限制
- 不偽造成功，不樂觀回報未驗證結果
