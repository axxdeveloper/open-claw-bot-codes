...
- 一般單題影片只挑一個主題與一個主要原始來源；Isaac Note 只能當內部線索，公開來源欄位必須寫原始來源，不寫 Isaac Note。例外是 Isaac 明確要求或固定授權的「每週技術雷達」格式：可以回顧一週多個技術來源，但 `reports/weekly-tech-radar/`、source-watch、HN、daily-audio-pack 只能當內部策展 artifact，公開 description/slides 必須列出被介紹的原始來源 URL，不能把內部 report 當公開來源。之後不要為影片排程自動產生 Isaac Note 文章、branch 或 PR；除非 Isaac 明確要求發文/開 PR，影片 workflow 只做原始來源影片與 YouTube 發布回報。
