...
export default function CreateTenantForm({ buildingId }: { buildingId: string }) {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await fetch(`/api/buildings/${buildingId}/tenants`, {
      method: "POST",
    ...
    if (!json.ok) {
      setError(json.error?.message || "新增租戶失敗");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-2 rounded border bg-white p-4 md:grid-cols-3">
      ...
      <button className="rounded bg-black px-3 py-2 text-white md:col-span-3">新增租戶</button>
    </form>
  );
}
