    ...
    <main className="page">
      <div className="card">
        ...
        <p className="muted" style={{ marginTop: 6 }}>
          修繕紀錄可依狀態、範圍、樓層、公共區域查詢。
        </p>
      </div>

      {error && <div className="errorBox">{error}</div>}

      <form className="card row" onSubmit={filter}>
        <select name="status">
      ...
      <form className="card row" onSubmit={createVendor} aria-label="create-vendor-form">
        <input name="name" placeholder="new vendor name" required />
        <button type="submit">Add Vendor</button>
      </form>

      <form className="card grid" onSubmit={createRepair} aria-label="create-repair-form">
        <div className="row">
        ...
        <div className="row">
          ...
          <select name="vendorId">
            <option value="">vendor id (optional)</option>
            {vendors.map((v) => (
